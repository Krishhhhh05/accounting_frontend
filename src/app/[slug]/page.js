...
function DynamicPage({ params }) {
  ...
  const fetchTypeOptions = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/get_type_options/${slug}`);
  ...
  const handleSubmitNow = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/submit_payment/', formData, {
        headers: {
  ...
  const checkPaymentStatus = async (dbName, collectionName, docId) => {
    try {
      const response = await axios.get(`http://localhost:8000/api/check_payment_status/${dbName}/${collectionName}/${docId}`);
